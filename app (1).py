...
# Load the dataset
df = pd.read_csv(r"C:\Users\darpa\Latest ODI Matches Till 2025 Updated.csv")
...
def make_group_table(df, group_by_col):
    # Keep only balls where isWide is not True
    temp_df = df[df["isWide"] != True]  # True = wide, everything else counts as legal
